...
| File Name             | Purpose                                         |
|-----------------------|-------------------------------------------------|
| `convert.py`          | Convert .m4a → 96 kHz WAV                       |
...
| `analyze_band.py`     | Analyze ultrasonic band energy (18–34 kHz)      |
...
- `hey_google_96k.wav` — Wake-word sample (converted)
- `noise.wav` — Noise sample (converted)
- `attack_am_strong_23k.wav` — (generated by script)
- `Normal1cam.m4a`, `Normalflash.m4a`, `Normalset.m4a` — Normal speech samples for false positive testing
...
3. Run the notebook cells in order — the notebook already imports and calls each `.py` module where needed  
...
## **▶ 3. Running the Codebase (.py Files)**
...
### **Step 2 — Generate Ultrasonic AM Attack**
File to run:  
...
### **Step 3 — Nonlinearity + Demodulation Simulation**
File to run:  
**demod_sim.py**
...
### **Step 4 — Wake-Word Similarity Check**
File to run:  
...
### **Step 5 — Ultrasonic Band Analysis**
File to run:  
**analyze_band.py**
...
### **Step 6 — Adaptive Ultrasonic Detector**
File to run:  
...
### **Step 7 — Defense System (Detect + Block)**
File to run:  
**detect_and_block.py**
...
### **Step 9 — ASR-Proxy Attack Evaluation**
File to run:  
...
### **Step 10 — ASR-Proxy After Defense**
File to run:  
...
### **Step 11 — Phase-2 Batch Evaluation**
File to run:  
**metrics_phase1.py**  
**phase2_batch.py**
...
### **Step 12 — Phase-3 Full Stress Testing**
File to run:  
...
## **💻 4. Running the Entire Project via Google Colab**
